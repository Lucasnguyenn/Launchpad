...
function ModalSuccessClaimComponent(props: {
  setIsVisibleModal: any;
  isVisibleModal: boolean;
  nativeClaim: any;
  tokenClaim: any;
}) {
  const { isVisibleModal, setIsVisibleModal, tokenClaim } = props;
                ...
                >
                  ...
                  <div className="w-fit mx-auto flex justify-center items-center gap-2 px-6 py-4 rounded-full bg-white mb-[56px] ">
                    <p className="text-[32px] leading-[32px] text-black font-semibold">
                      {`${formatDisplay(Number(tokenClaim), {
                        decimalToShow: AppConstant.TOKEN_RAISE.DECIMAL_TO_SHOW,
                      })} ${projectDetail.tokenIssue}`}
                    </p>
...
export const ModalSuccessClaim = memo(ModalSuccessClaimComponent, isEqual);
